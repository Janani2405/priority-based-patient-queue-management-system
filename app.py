...
ODBC_CONNECTION_STRING = "DSN=MyOracleDB;Uid=system;Pwd=j2406;"
...
def triage():
    ...
    if 'user_id' in session and session.get('role') == 'Patient':
        ...
        if not name or not contact:
            cursor.execute("SELECT NAME, AGE, GENDER, CONTACT FROM PATIENT_TRIAGE WHERE PATIENT_ID = ? FETCH FIRST 1 ROWS ONLY", (patient_id,))
            row = cursor.fetchone()
            if row:
...
def queue():
    ...
    specialization = patient_details[3]
    assigned_doctor_id = patient_details[5]
    appointment_date = patient_details[6]  # Add this to capture the logged-in patient's appointment date

    cursor.execute("""
        SELECT P.PATIENT_ID, P.NAME, U.NAME AS DOCTOR_NAME, 
            P.SPECIALIZATION, TO_CHAR(P.APPOINTMENT_DATE, 'YYYY-MM-DD'),
            P.APPOINTMENT_TIME, P.TRIAGE_LEVEL, P.ESTIMATED_WAITING_TIME
        FROM PATIENT_TRIAGE P
        LEFT JOIN USERS1 U ON P.ASSIGNED_DOCTOR = U.USER_ID  
        WHERE P.STATUS IS NOT NULL AND P.STATUS != 'Diagnosed' AND P.STATUS != 'Cancelled'
        AND P.SPECIALIZATION = ? 
        AND P.ASSIGNED_DOCTOR = ?
        AND TO_CHAR(P.APPOINTMENT_DATE, 'YYYY-MM-DD') = ?
        ORDER BY 
            CASE P.TRIAGE_LEVEL 
                WHEN 'Critical' THEN 1
                WHEN 'High' THEN 2
    ...
    """, (specialization, assigned_doctor_id, appointment_date))
...
def emergency():
    if request.method == 'POST':
        ...
        try:
            ...
            # STEP 3: Emergency doctor (assign regardless of appointment count)
            if not assigned_doctor:
                cursor.execute("""
                    SELECT DOCTOR_ID 
                    FROM DOCTOR_SCHEDULE 
