...
CREATE TABLE PATIENT_TRIAGE (
    PATIENT_ID VARCHAR2(10) PRIMARY KEY,  -- Stores 'P001', 'P002', etc.
    ...
    TRIAGE_LEVEL VARCHAR2(50),
    SUGGESTED_DEPARTMENT VARCHAR2(50),
    ESTIMATED_WAITING_TIME VARCHAR2(50),
...
ALTER TABLE PATIENT_TRIAGE ADD STATUS VARCHAR2(20) DEFAULT 'Pending';
ALTER TABLE PATIENT_TRIAGE MODIFY APPOINTMENT_DATE DATE;
ALTER TABLE PATIENT_TRIAGE add APPOINTMENT_DATE DATE;
ALTER TABLE PATIENT_TRIAGE ADD ASSIGNED_DOCTOR VARCHAR2(100);
ALTER TABLE PATIENT_TRIAGE
ADD (BOOKING_DATE DATE, BOOKING_TIME TIMESTAMP);
ALTER TABLE PATIENT_TRIAGE
...
ALTER TABLE PATIENT_TRIAGE ADD BP VARCHAR2(20);
ALTER TABLE PATIENT_TRIAGE ADD HEART_RATE NUMBER;
ALTER TABLE PATIENT_TRIAGE ADD RESPIRATORY_RATE NUMBER;
ALTER TABLE PATIENT_TRIAGE ADD OXYGEN_SATURATION NUMBER;
ALTER TABLE PATIENT_TRIAGE ADD EMERGENCY_LEVEL VARCHAR2(50);

ALTER TABLE USERS1 ADD SPECIALIZATION VARCHAR2(50);
ALTER TABLE PATIENT_TRIAGE ADD SPECIALIZATION VARCHAR2(50);

ALTER TABLE PATIENT_TRIAGE DROP COLUMN SUGGESTED_DEPARTMENT;
ALTER TABLE PATIENT_TRIAGE ADD SPECIALIZATION VARCHAR2(50);
ALTER TABLE PATIENT_TRIAGE ADD APPOINTMENT_TIME VARCHAR2(20);
ALTER TABLE PATIENT_TRIAGE ADD  BOOKING_DATETIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

ALTER TABLE PATIENT_TRIAGE MODIFY CONTACT NUMBER(15)
ALTER TABLE PATIENT_TRIAGE ADD EMERGENCY_LEVEL VARCHAR2(50);
--------------------------------------------------------------------------------
CREATE TABLE USERS1 (
...
SELECT PATIENT_ID, NAME, ASSIGNED_DOCTOR FROM PATIENT_TRIAGE;
...
-- ? Create Trigger to Auto-Increment SCHEDULE_ID
CREATE OR REPLACE TRIGGER doctor_schedule_trigger 
BEFORE INSERT ON DOCTOR_SCHEDULE 
FOR EACH ROW
BEGIN
    SELECT doctor_schedule_seq.NEXTVAL INTO :NEW.SCHEDULE_ID FROM DUAL;
END;
/
drop TRIGGER doctor_schedule_trigger;
desc doctor_schedule;
------------------------------

UPDATE doctor_schedule
SET Available_days = 'Monday, Wednesday, Saturday'
WHERE  doctor_id='D05';

Delete from users1 where user_id='P003';
...
ALTER TABLE PATIENT_TRIAGE DROP COLUMN VITAL_SIGNS;

ALTER TABLE PATIENT_TRIAGE DROP COLUMN PRIORITY_SCORE;

ALTER TABLE PATIENT_TRIAGE DROP COLUMN emergency_level;
...
UPDATE PATIENT_TRIAGE SET APPOINTMENT_DATE = TRUNC(SYSDATE);
UPDATE PATIENT_TRIAGE SET APPOINTMENT_DATE ='26-05-25';

UPDATE PATIENT_TRIAGE SET STATUS = 'Pending';

Delete from PATIENT_TRIAGE where Name='Maran';
